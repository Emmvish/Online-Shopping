...
test('Should obtain payout value for seller', async () => {
    const response = await request(app).post('/payout').set('Authorization', `Bearer ${userTwo.tokens[0].token}`).send().expect(201);
    expect(response.body.monthlyEarnings).toBe(500);
})

test('Should NOT obtain payout value for non-seller', async () => {
    const response = await request(app).post('/payout').set('Authorization', `Bearer ${userThree.tokens[0].token}`).send().expect(400);
